...
const { assertPersistableAnalysis } = require('./dev2vec/dev2vecPersistence.service');
const {
...
const createSnapshotFromAnalysisResult = async (analysisResult) => {
  ...
  assertPersistableAnalysis(toObject(analysisResult));
...
const formatSnapshotResponse = (snapshotInput, options = {}) => {
  ...
  const response = {
    ...
    repoDocumentVersion: compatibility.repoDocumentVersion,
    issueDocumentVersion: compatibility.issueDocumentVersion,
    apiEvidenceVersion: compatibility.apiEvidenceVersion,
...
const compareSnapshots = async (user, { fromSnapshotId, toSnapshotId, snapshotAId, snapshotBId } = {}) => {
  ...
  const versionComparison = compareSnapshotVersions(fromSnapshot, toSnapshot);
  if (!versionComparison.compatible) {
    const error = createStatusError('Snapshots were generated by different Dev2Vec versions.', 409);
    error.errorCode = 'incompatible_snapshot_versions';
    error.comparisonStatus = 'incompatible_snapshot_versions';
    error.leftVersion = versionComparison.leftVersion;
    error.rightVersion = versionComparison.rightVersion;
    error.versionDifferences = versionComparison.differences;
    error.errors = [{
      comparisonStatus: error.comparisonStatus,
      leftVersion: error.leftVersion,
      rightVersion: error.rightVersion,
      versionDifferences: error.versionDifferences,
    }];
    throw error;
  }
...
const compareRepositoryProgress = async (user, repoId) => {
  validateAuthUser(user);
  const repository = await findRepositoryForUser(user, repoId);
  const snapshots = await RepoAnalysisSnapshot.find(buildCompatibleSnapshotQuery({
    userId: user.userId,
    repositoryId: repository._id,
  }))
  ...
  if (snapshots.length < 2) {
    return {
      ...
      data: {
        ...
        analysisScopeType: 'user_contribution',
        enoughData: false,
        comparisonStatus: 'insufficient_compatible_snapshots',
        snapshotsCount: snapshots.length,
        comparison: null,
        comparisonVersion: getCurrentDev2VecVersions(),
      },
      statusCode: 200,
    };
  }
  ...
  return {
    ...
    data: {
      ...buildComparisonResult(snapshots[0], snapshots[snapshots.length - 1]),
