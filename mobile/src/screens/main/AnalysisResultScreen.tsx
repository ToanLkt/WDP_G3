...
import { AlertCircle, BookOpen, Layers, GitCommit, FileText, CheckCircle2, ChevronRight, MessageSquareCode, Milestone } from 'lucide-react-native';
...
export const AnalysisResultScreen: React.FC = () => {
  ...
  const fetchAnalysis = async () => {
    ...
    setError('');
    try {
      const res = await mockFetchAnalysisResult(repoId);
      setData(res);
    } catch (err: any) {
  ...
  const handleConsultAI = () => {
    // Navigate to Chat Tab passing context
  ...
  const handleGenerateRoadmap = async () => {
    ...
    try {
      await generateRoadmapAction(repoId);
      navigation.navigate('RoadmapTab');
    } catch (err) {
      alert('Không thể tạo lộ trình học tập. Vui lòng thử lại!');
    } finally {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <Card style={styles.titleCard} glow="violet">
        <Text style={styles.projType}>{data.project_type.toUpperCase()}</Text>
        ...
        <Button
          ...
          variant="secondary"
          style={{ marginTop: theme.spacing.md, height: 40 }}
          icon={<Milestone size={14} color="#FFF" style={{ marginRight: 6 }} />}
        />
      </Card>
